      ...
      <View style={{ paddingHorizontal: 10, paddingVertical: 15 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text style={{ color: "grey", fontSize: 12 }}>
            {item.status == "o" && "Open "}
            {item.status == "o" && getTimeDifference2(item.postdate)}
            {item.status == "c" && "Closed "}
            {item.status == "c" && getTimeDifference2(item.closedate)}
          </Text>
        ...
        <Text
          style={{
            fontWeight: "500",
            color: selectedItem === item.id ? "#1E319D" : "#404040",
            letterSpacing: 1,
          }}
        >
          {capitalizeWords(item.job_title)} - {capitalizeWords(item.nature)}
        </Text>
        <Text style={{ marginTop: 5, color: "#404040" }} numberOfLines={7}>
          {capitalizeFirstLetterOfParagraphs(item.job_description)}
        </Text>
        ...
        >
          <Text style={{ color: "#404040" }}>
            ...
            <Text style={{ color: "grey" }}>
              {capitalizeWords(item.location_)}
            </Text>
          </Text>
          <Text style={{ color: "#404040" }}>
            Slot:{" "}
            <Text style={{ color: "grey" }}>
              {capitalizeWords(item.vacancy_no)}
            </Text>
          </Text>
        </View>
      </View>
